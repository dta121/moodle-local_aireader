...
/**
 * External-function tests for local_aireader_set_position security gates.
 *
...
/**
 * Tests for {@see set_position}.
 *
 * @coversDefaultClass \local_aireader\external\set_position
 */
final class set_position_test extends \advanced_testcase {
    /**
     * Hidden-chapter assets must not accept playback writes from listeners who
     * cannot view the hidden chapter.
     *
     * @covers ::execute
     */
    public function test_rejects_hidden_chapter_asset_for_student(): void {
        ...
        set_position::execute((int)$asset->id, 10);
    }

    /**
     * Teachers who can view hidden Book chapters may store their own position.
     *
     * @covers ::execute
     */
    public function test_allows_hidden_chapter_asset_for_teacher(): void {
        ...
        $result = set_position::execute((int)$asset->id, 10);
        $this->assertTrue($result['success']);
        $this->assertTrue($DB->record_exists('local_aireader_position', ['assetid' => $asset->id]));
    }
